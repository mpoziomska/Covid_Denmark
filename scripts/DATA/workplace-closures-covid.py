...
with open('../data/workplace-closures-covid.csv') as f:
    p = pd.read_csv(f)

with open('../data/workplace-closures-covid.txt', 'w') as f:
    ...
    f.writelines("There may be sub-national or regional differences in policies on workplace closures. The policy categories shown may not apply at all sub-national levels. A country is coded as ‘required closures’ if at least some sub-national regions have required closures.")
