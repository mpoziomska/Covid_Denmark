...
with open('../data/public-events-covid.csv') as f:
    p = pd.read_csv(f)

with open('../data/public-events-covid.txt', 'w') as f:
    ...
    f.writelines("There may be sub-national or regional differences in policies on event cancellations. The policy categories shown may not apply at all sub-national levels. A country is coded as ‘required’ if at least some sub-national regions have required cancellations.")
